...
model = keras.models.load_model('/home/majid/Desktop/model/model.h5')
...
for image_path in images:
    ...
    input_data = np.float32(image)  / 127.5 - 1.0
    img = tf.keras.utils.load_img(
    os.path.join(CWD_PATH,IM_NAME), target_size=(height, width)
        )
    input_data2 = tf.keras.utils.img_to_array(img)
    input_data2 = tf.expand_dims(input_data2, 0)    
    ...
    for i in range(len(scores)):
        ...
        print('Predictions after prediction :\n', predictions)
        ...
        cv2.putText(image , "bobcat", (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2) # Draw label text
